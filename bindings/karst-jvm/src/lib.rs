...
use jni::sys::{jint, jlong, jobject, jstring, JNI_VERSION_1_8};
...
#[no_mangle]
pub extern "C" fn JNI_OnLoad(vm: JavaVM, _reserved: usize) -> jint {
...
#[no_mangle]
pub extern "C" fn Java_org_karst_Karst_createProducer(
    env: JNIEnv,
    _klass: JClass,
    bootstrap: JString,
) -> jlong {
    let bootstrap: String = env
        .get_string(bootstrap)
        .expect("Failed to get bootstrap string")
        .into();
    ...
    match blocking_await(res) {
        ...
        Ok(Err(e)) => {
            java_throw(&env, &format!("Connect error: {:#}", e));
            -1
        }
        Err(e) => {
            java_throw(&env, &format!("Join error: {:#}", e));
            -1
        }
    }
}
...
#[no_mangle]
pub extern "C" fn Java_org_karst_Karst_producerSendString(
    env: JNIEnv,
    _klass: JClass,
    producerId: jlong,
    topic: JString,
    ...
    let key: String = env
        .get_string(key)
        .expect("Getting key string failed")
        .into();
    let value: String = env
        .get_string(value)
        .expect("Getting value string failed")
        .into();
    let topic: String = env
        .get_string(topic)
        .expect("Getting topic string failed")
        .into();
    let mut res = TOKIO_RUNTIME.spawn(async move {
...
#[no_mangle]
pub extern "C" fn Java_org_karst_Karst_producerClose(
    env: JNIEnv,
    _klass: JClass,
    producer_id: jlong,
) {
...
fn java_throw(env: &JNIEnv, msg: &str) {
    env.throw_new("org/karst/KarstException", msg)
